...
def _resolve_embedding_model_name(name: Optional[str]) -> Optional[str]:
    ...
    mapping = {
        ...
        "bge-flagembedding": "bge-base-en-v1.5",
        "bge_flagembedding": "bge-base-en-v1.5",
        "bge_flagembeddings": "bge-base-en-v1.5",
        "flagembedding": "bge-base-en-v1.5",
        "flagembeddings": "bge-base-en-v1.5",
...
def upsert_doc_cooc(cx: sqlite3.Connection, doc_id: str, vocab: dict, rows, row_norms):
    """
    vocab: {token: row_id}
    rows:  sparse CSR (PPMI or normalized); pass rows.data/indices/indptr/shape
    row_norms: dict or numpy array of L2 norms per row
    """
    _ensure_schema(cx)
    
    # Validate that we have scipy and CSR matrix
    if sp is None:
        raise RuntimeError("scipy.sparse required for cooc rows but not available")
    ...
    if not sp.isspmatrix_csr(rows):
        try:
            ...
            raise RuntimeError(f"Cannot convert rows to CSR matrix: {e}")
    
    # Store vocab as JSON
    cx.execute("REPLACE INTO indices(doc_id, kind, payload) VALUES (?,?,?)",
               (doc_id, "cooc_vocab", _blob_from_json({"vocab": vocab})))
    
    # Convert row_norms to numpy array if needed
    if isinstance(row_norms, dict):
        # Convert dict {idx: norm} to array
        max_idx = max(row_norms.keys()) if row_norms else 0
        norms_array = np.zeros(max_idx + 1, dtype=np.float32)
        for idx, norm in row_norms.items():
            norms_array[idx] = norm
    elif isinstance(row_norms, (list, tuple)):
        norms_array = np.array(row_norms, dtype=np.float32)
    else:
        norms_array = np.asarray(row_norms, dtype=np.float32)
    ...
    blob = _blob_from_npz(
        ...
        indptr=rows.indptr,
        shape=np.array(rows.shape, dtype=np.int32), 
        norms=norms_array
    )
    cx.execute("REPLACE INTO indices(doc_id, kind, payload) VALUES (?,?,?)",
