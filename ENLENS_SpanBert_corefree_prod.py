...
def run_quick_analysis(
    ...
    **kwargs
    ...
    # ---- normalize cooc knobs ----
    effective_cooc_mode = (cooc_mode or kwargs.get("cooc_mode") or "hf")
    effective_hf_tok = cooc_hf_tokenizer or kwargs.get("cooc_hf_tokenizer")
    effective_window = int(cooc_window or kwargs.get("cooc_window", 5))
    effective_min_count = int(cooc_min_count or kwargs.get("cooc_min_count", 2))
    effective_topk_neighbors = int(cooc_topk_neighbors or kwargs.get("cooc_topk_neighbors", 10))
    effective_impl = kwargs.get("cooc_impl", "fast")
    effective_max_tokens = int(kwargs.get("cooc_max_tokens", 50000))

    if shortlist_mode in ("cooc", "both"):

        return_csr = kwargs.get("return_csr", False)
        
        vocab, rows, row_norms = build_cooc_graph(
            full_text,
            window=effective_window,
            min_count=effective_min_count,
            topk_neighbors=effective_topk_neighbors,
            mode=effective_cooc_mode,
            hf_tokenizer=effective_hf_tok,
            cooc_impl=effective_impl,
            cooc_max_tokens=effective_max_tokens,
            cache_key=hash(full_text) % (10**9),      
            return_csr=return_csr,
        )
    else:
        vocab, rows, row_norms = {}, None, {}
    ...
    for idx, sent in enumerate(sentences):
        ...
        for rank, sp in enumerate(spans, start=1):
            ...
            extra_shortlist = {}
            if shortlist_mode in ("trie", "both"):
                extra_shortlist["trie_candidates"] = [
                {"chain_id": cid, "score": float(sc)} for cid, sc in triers
                ]
                
            if shortlist_mode in ("cooc", "both"):
                extra_shortlist["cooc_candidates"] = [
                {"chain_id": cid, "score": float(sc)} for cid, sc in coocs
                ]

            coref_payload = (
                    {
                    **coref_info,
                    "decision": ("shortlist+scorer" if pair_scorer else "shortlist"),
            ...
            span_items.append({
                ...
                "bbox": mapped_bbox,              
                "coreference_analysis": coref_payload, 
                #(
