...
def _embed_tokens(tokens, vocab, rows, row_norms):
    acc = defaultdict(float)
    ids = [vocab.get(t) for t in tokens if t in vocab]
    if sp.isspmatrix_csr(rows):
        for i in ids:
            if i is None or i >= rows.shape[0]:
                continue
            start, end = rows.indptr[i], rows.indptr[i+1]
            for j, val in zip(rows.indices[start:end], rows.data[start:end]):
                acc[j] += val
    else:
        for i in ids:
            if i is None:
            ...
            for j, val in rows[i].items():
                acc[j] += val
    norm = (sum(v*v for v in acc.values()) ** 0.5) or 1.0
