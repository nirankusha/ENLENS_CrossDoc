...
try:  # Optional: shared helpers for co-occ backend resolution
    from cooc_utils import resolve_cooc_backend as _resolve_cooc_backend
except Exception:  # pragma: no cover - cooc_utils may be absent in slim deployments
    _resolve_cooc_backend = None
...
def run_ingestion_quick(
    ...
    **extra,
    ...
    cooc_warn: Optional[str] = None
    # Normalise HuggingFace tokenizer inputs: callers may provide a name or an instance.
    if _resolve_cooc_backend is not None and (
        cooc_hf_tokenizer is None or isinstance(cooc_hf_tokenizer, (str, bytes))
    ):
        cfg = {
            "cooc_mode": cooc_mode,
            "cooc_hf_tokenizer": cooc_hf_tokenizer.decode()
            if isinstance(cooc_hf_tokenizer, bytes)
            else cooc_hf_tokenizer,
        }
        try:
            resolved_mode, resolved_tok, warn = _resolve_cooc_backend(cfg)
            cooc_mode = resolved_mode
            cooc_hf_tokenizer = resolved_tok
            cooc_warn = warn
        except Exception as exc:  # pragma: no cover - defensive: HF/transformers failures
            logger.warning("Failed to resolve co-occ backend: %s", exc)
            cooc_hf_tokenizer = None
            cooc_warn = f"Failed to resolve co-occ backend: {exc}"
    # -------------------------------------------------
    # 1) choose pipeline
    # -------------------------------------------------
    if candidate_source == "kp" and _run_quick_kpe is not None:
        ...
        # map old "shortlist_mode" -> "coref_shortlist_mode"
        ...
        raw_kwargs = dict(
            ...
            top_k_phrases=kpe_top_k or top_k_spans,
        ...
        raw_kwargs["cooc_impl"] = "fast"
        raw_kwargs["return_csr"] = True
        
        # --- NEW: pass co-occ knobs down to pipeline ---
        if cooc_mode is not None:
            raw_kwargs["cooc_mode"] = cooc_mode
        if cooc_hf_tokenizer is not None:
            # keep the real object, not its name
            raw_kwargs["cooc_hf_tokenizer"] = cooc_hf_tokenizer

        # also propagate numeric knobs that user/UI put into **extra
        if "cooc_window" in extra:
            raw_kwargs["cooc_window"] = extra["cooc_window"]
        if "cooc_min_count" in extra:
            raw_kwargs["cooc_min_count"] = extra["cooc_min_count"]
        if "cooc_topk_neighbors" in extra:
            raw_kwargs["cooc_topk_neighbors"] = extra["cooc_topk_neighbors"]
        if "cooc_impl" in extra:
            raw_kwargs["cooc_impl"] = extra["cooc_impl"]
        if "return_csr" in extra: 
            raw_kwargs["return_csr"] = extra["return_csr"]    
        # some UI / app fields that we want to preserve if given
        _whitelist = {
    ...
    if cooc_warn:
        if isinstance(result, dict):
            result.setdefault("_warn", []).append(cooc_warn)
    # Normalize: always put production dict in "production_output"
    prod = _unwrap_production(result) or result
    if cooc_warn and isinstance(prod, dict):
        prod.setdefault("_warn", []).append(cooc_warn)
    try:
    ...
    if db_path:
        ...
        if callable(build_ngram_trie) and callable(build_cooc_graph):
            ...
            try:
                ...
                # Resolve co-occurrence backend settings
                ...
                try:
                    ...
                    # Validate and store co-occurrence data
                    if vocab and rows is not None:
                        try:
                            import scipy.sparse as sp
                            if sp.issparse(rows):
                                upsert_doc_cooc(cx, doc_id, vocab, rows, row_norms)
                                logger.info(f"✅ Stored co-occurrence graph: {len(vocab)} tokens, {rows.nnz} edges")
                            else:
                                logger.warning("Co-occurrence rows are not sparse; skipping export.")
                        except ImportError:
                            logger.warning("SciPy not available; skipping co-occurrence export.")
                    else:
                        logger.warning("Co-occurrence builder returned empty results.")
                        
                except Exception as e:
                    logger.warning(f"Co-occurrence graph build failed: {e}")
                    import traceback
                    traceback.print_exc()
